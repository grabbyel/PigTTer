...
tweetsRouter.get('/', async (request, response) => {
        ...
        const updatedTweets = await Tweet.find({}).populate('comments').populate('user', {username: 1, name: 1, image: 1})
        response.json(updatedTweets)
})

tweetsRouter.get('/:id', async (request, response) => {
    const tweet = await Tweet.findById(request.params.id).populate('comments').populate('user', {username: 1, name: 1, image: 1})
    response.json(tweet)
})
...
tweetsRouter.put('/:id', async (request, response) => {
    ...
    if(tweet.hasOwnProperty('username')){
        ...
        const updatedTweet = await Tweet.findByIdAndUpdate(request.params.id, objectToUpdate, { new: true, runValidators: true, context: 'query' })
